...
// Seeds that fall out after animation — the fruits of the learning loop
const SEEDS = [
  { word: "DOSAH", sub: "reach" },
  { word: "PRODEJ", sub: "conversions" },
  { word: "ÚSPĚCH", sub: "results" },
  { word: "RŮST", sub: "growth" },
]
...
export function SeedOfLife() {
  ...
  // Cycle seed words every 2.5s after animation complete
  useEffect(() => {
    if (!animationComplete) return
    const interval = setInterval(() => {
      setActiveSeedIndex(i => (i + 1) % SEEDS.length)
    }, 2500)
  ...
  return (
    <div ref={sectionRef} className="flex flex-col items-center gap-10 max-w-5xl mx-auto">
      ...
      <div className="relative">
        <div className="relative w-[340px] h-[340px] md:w-[460px] md:h-[460px]">
          <svg viewBox="0 0 420 420" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
            <defs>
              ...
              {/* Seed shape — vesica piscis / teardrop */}
              <filter id="seed-glow">
              ...
              return (
                <g key={i}>
                  ...
                  <motion.circle cx={c.x} cy={c.y} r={r}
                    fill="none"
                    stroke={isHovered ? "#e63946" : isCenter ? "rgba(230,57,70,0.8)" : "rgba(255,255,255,0.14)"}
                  ...
                  <AnimatePresence>
                    {visible && (isHovered || (!animationComplete && activeStep === i)) && (
                      ...
                      >
                        <text x={c.x} y={c.y - 5} textAnchor="middle" dominantBaseline="middle"
        ...
        {/* FALLING SEED — appears after animation complete, below the flower */}
        <AnimatePresence>
          {animationComplete && (
            <motion.div
              key="seed-container"
            ...
            >
              {/* Seed SVG shape — vesica piscis / eye shape */}
              <div className="relative flex flex-col items-center">
                {/* Connecting line from flower to seed */}
                <motion.div
                  className="w-px bg-gradient-to-b from-aisummit-cinnabar/40 to-aisummit-cinnabar/10 h-8"
                  initial={{ scaleY: 0 }}
                  animate={{ scaleY: 1 }}
                ...
                >
                  {/* Seed SVG */}
                  <svg width="80" height="80" viewBox="0 0 80 80" className="drop-shadow-[0_0_12px_rgba(230,57,70,0.5)]">
                    <defs>
                      <radialGradient id="seed-fill" cx="50%" cy="40%" r="60%">
                        <stop offset="0%" stopColor="#e63946" stopOpacity="0.3" />
                        <stop offset="100%" stopColor="#e63946" stopOpacity="0.05" />
                      </radialGradient>
                    </defs>
                    {/* Vesica piscis shape — two arcs forming an eye/seed */}
                    <path
                      d="M40 10 C60 10, 70 25, 70 40 C70 55, 60 70, 40 70 C20 70, 10 55, 10 40 C10 25, 20 10, 40 10 Z"
                      fill="url(#seed-fill)"
                      stroke="#e63946"
                      strokeWidth="1.5"
                    />
                    {/* Inner seed shape */}
                    <path
                      d="M40 20 C52 20, 58 30, 58 40 C58 50, 52 60, 40 60 C28 60, 22 50, 22 40 C22 30, 28 20, 40 20 Z"
                      fill="none"
                      stroke="rgba(230,57,70,0.3)"
                      strokeWidth="1"
                    />
                    {/* Center dot */}
                    <circle cx="40" cy="40" r="3" fill="#e63946" opacity="0.8" />
                  </svg>

                  {/* Pulsing glow ring around seed */}
                  <motion.div
                    className="absolute inset-0 rounded-full border border-aisummit-cinnabar/20"
                ...
                {/* Cycling result word */}
                <div className="mt-3 h-12 flex flex-col items-center justify-center">
                  <AnimatePresence mode="wait">
                    ...
                    >
                      <div className="text-aisummit-cinnabar text-sm font-black uppercase tracking-[0.2em]">
                        {SEEDS[activeSeedIndex].word}
                      </div>
                      <div className="text-white/25 text-[9px] font-bold uppercase tracking-widest mt-0.5">
                        {SEEDS[activeSeedIndex].sub}
                      </div>
                    </motion.div>
                  </AnimatePresence>
                </div>
