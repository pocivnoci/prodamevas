...
16. **Kredity — media-weighted (v6.9, reel-duration v8.1)**: post stojí `creditsForMedia(medium, reelDurationSec?)` = **image 1 / carousel 3 / reel od 5** (`lib/credits.ts` — client-safe zdroj pravdy, `lib/subscription.ts` re-exportuje; UI odhady importují z `lib/credits`). **Reel je nově účtovaný podle délky**: `creditsForReel(sec)` = `5 + 4×(další započatý 8s blok)` → 8 s = 5, 16 s = 9, 24 s = 13 (multi-clip reels se stříhají po 8s blocích, každý blok ≈ Veo fast $1.20). Charge při `ig-create-job` (ne po generování): účtované médium = explicitní `body.medium`, jinak formát post typu (`getPostFormat`), s pre-aplikací kill-switche a plan gatingu; u reelu se navíc odvodí **`chargedReelDuration`** z `getReelDuration(type, config)`. `ig_jobs.config` nese `charged` ('plan'/'credits'/'none') + **`chargedCredits`** + **`chargedMedium`** + **`chargedReelDuration`**. Engine dostává `options.chargedMedium`+`chargedReelDuration` a **nikdy nevyrenderuje dražší médium ani delší reel** (rank clamp + duration clamp v `applySafetyClamps`); levnější/kratší výstup se po úspěchu dorovná refundem přes `reconcileJobCharge(..., actualReelDurationSec)` (action `post_adjust`, idempotentní přes unique index). Refund při selhání vrací přesně `chargedCredits`. Kampaně: worker účtuje per-item médium (pre-clamped), `startCampaign` dělá weighted up-front check (Σ creditsForMedia) + per-item reel gating. **Varianty** (`generatePostVariant`) se nově účtují (`post_variant`, weighted podle média originálu — dřív běžely ZDARMA, billing leak); revize (`revisePost`) zůstává zdarma (feedback = learning signál). Plány re-budget: Start 20 / Růst 45 / Dominance 110 kreditů (`20260702_media_weighted_credits.sql`); trial má nově `allowed_media: image+carousel` (žádné reels zdarma).
...
27. **Publikování na Instagram (roadmap step 2)**: post se publikuje přes `/api/cron/ig-publisher` (cron každou minutu, `vercel.json`). Stav-stroj: `draft`/`ready` → (uživatel schválí čas přes `schedulePostAction`) → **`scheduled`** (armed) → cron atomicky flipne `scheduled→posting` (claim přes podmíněný UPDATE na `status` — zabrání double-publish dvou ticků) → `posted` (+ `ig_media_id`, `permalink`, `posted_at`) nebo `failed` (+ `publish_error`, po `MAX_ATTEMPTS=4` pokusech s exponenciálním backoffem). Graph volání je `instagramAdapter.publish()` v `lib/channels/instagram.ts` (container→publish; **image + carousel + reel**). **Reels (v8.1):** REELS container (`media_type=REELS`, `video_url`, `cover_url`/`thumb_offset`, `share_to_feed`), delší poll window (~5 min, `waitForContainer` opts — Meta video transcoduje serverově). Jen `"video"` médium ještě hodí `ChannelNotEnabledError`. Carousel slidy zůstávají pipe-joined v `ig_posts.image_url` (`url1|url2|...`); reel je `video|cover` — publisher i UI čtou přes **`parsePostMedia(image_url, media_type)`** (`lib/media-urls.ts`, `media_type` vyhrává nad pipe-sniffingem; jediný povolený parser, žádný raw `split("|")` mimo tento modul). `media_type` se ukládá při generování (`autopilot.ts` createPost). `schedulePostAction`/`retryPublishAction` guardují na `connected` řádek v `ig_connections`. **Vyžaduje scope `instagram_business_content_publish` = 2. App Review submission** — do schválení funguje jen pro účet, jehož přihlašující uživatel je admin/tester Meta appky (= dogfood). Žádné účtování kreditů v publisheru (kredity se berou při generování). Migrace: `supabase/migrations/20260622_ig_publishing.sql`.
   ...
   - **⚡ Publikovat hned (instant auto-publish z handoff modalu):** když je účet **připojený** a post je image/carousel/**reel** (v8.1 — `canAutoPublish` už reely nevylučuje, jen vyžaduje video URL), modal nabídne `publishNowAction` (`calendar-actions.ts`) jako primární akci — armne post (`status='scheduled'` + `scheduled_for=now`), `ig-publisher` cron ho vydá (reel déle — video transcode), UI pollne `getPostPublishStatus` na „Publikováno"+permalink / „Selhalo". **Žádné synchronní Graph volání** (carousel/reel container-polling by jinak mohl timeoutnout server action — cron na to má 800s budget). Reuse publisher path = zero risk. Manuální Web Share zůstává sekundární (u reelu sdílí/stáhne MP4 + cover; pro nepřipojené účty jediná cesta). `PostsTab` předává `connected` přes `getConnectionStatus`. iOS Zkratka (volitelná, jen manuální cesta — IG **neumožní** auto-post ze zkratky): `docs/IOS_SHORTCUT_HANDOFF.md`.
...
45. **Reels — smysluplné video (v8.1)**: reel pipeline byla přepsaná z „surové Veo video bez zvuku" na plnou produkci (`instagram/orchestrators/reel-orchestrator.ts`). (a) **Kritický bug fix**: `buildVideoSchema` (`caption-generator.ts`) NEDEKLAROVAL `narration`/`soundEffect` ve scénách → structured output je nikdy negeneroval → voiceover + titulky + celý ffmpeg krok se TIŠE přeskakovaly. `narration` je teď required s rozpočtem slov v popisu (~2,5 slova/s délky scény — drží TTS uvnitř délky videa). (b) **TTS gotcha**: Gemini TTS vrací **headerless PCM (L16, 24 kHz)**, ne WAV — `generateVoiceover` ho teď balí přes `pcmToWav` (`video-processor.ts`), jinak by ffmpeg mix pokaždé spadl. (c) **Titulky = ffmpeg `drawtext` s `fontfile=`, NE libass `subtitles=`** — bare lambda nemá fontconfig; wrapping (`wrapSubtitleText`) se dělá v JS, jeden drawtext na řádek. Font `instagram/assets/fonts/Inter-Bold.ttf` (OFL, česká diakritika) + ffmpeg-static binárka MUSÍ být v `outputFileTracingIncludes` (`next.config.ts`) — binárka se spawnuje, NFT ji netrasuje. (d) **Multi-clip**: `planClips(sec)` rozřeže délku na ≤8s bloky (Veo cap), `groupScenesIntoClips` rozdělí scény, klipy se generují **paralelně** (`Promise.all`, vejde se do jednoho Veo wall-clocku) a ffmpeg je slepí (`processReelVideo` = concat + audio mix + titulky přes globální timeline → deterministický IG-spec MP4 H.264/AAC). (e) **Frame QA**: `extractFrames` (3 snímky/klip) → `verifyReelClip` přes `judgeVision` Pro ladder (fail-open, stejně jako `verifyNativeImage`); `severe` → 1 regenerace jen vadných klipů (jen když zbývá rozpočet ~330s z 800s), pak ship-best; `qa_status` reuse stringů `pass`/`retry_pass`/`native_forced`. (f) **Produkt grounding**: `loadProductPhoto` (`orchestrators/media-refs.ts`, sdílený s image-orchestrátorem) → produktová fotka je **první** ve Veo referencích (cap 3), produkt jde do `refineVideoPrompt`, do cover briefu i do QA. (g) **`refineVideoPrompt`** běží na **Pro ladderu** (`textPro`, ne flash — pravidlo „quality over fallback"), s clip-continuity kotvou u multi-clipu a web-brandingem jen na posledním klipu. (h) **Veo hard fail ⇒ 1 retry stejný tier, pak THROW** (job selže → refund) — konec tichých postů s prázdným `image_url`. (i) **Storage**: `audit-screenshots/ig-reels/<clientUuid>/<ts>.mp4` + `-cover.webp` (per-client buckety odmítají `video/mp4` MIME). `image_url = "video|cover"` — čte se přes `parsePostMedia`. (j) **UI**: `ReelPlayer` (`tabs/shared.tsx`) v modalech, thumbnaily = cover (nikdy .mp4), `ttsVoice` + délka reelu v SettingsTab formátech; `reelsEnabled` (z `/api/subscription`, čte `REELS_ENABLED`) řídí picker („Brzy" dokud kill-switch off). Testy: `scripts/test-reel-logic.ts` (clip planning, subtitle timing/escaping, WAV, kredity, `parsePostMedia`). Kill-switch `REELS_ENABLED` beze změny sémantiky.
...
│   ├── reel-orchestrator.ts          # ~380 LOC — multi-clip Veo + voiceover + titulky + frame QA + cover
│   ├── media-refs.ts                 # loadProductPhoto (sdílený image + reel grounding)
...
├── video-processor.ts                # ~430 LOC — ffmpeg: multi-clip concat, audio mix, drawtext titulky, PCM→WAV, frame extract
