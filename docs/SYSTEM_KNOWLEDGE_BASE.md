...
| `subscription_plans` | `id`, `name`, `price_czk`, `features` | Plan definitions — aktuální seed je **v6** (`20260901_pricing_v6_kredity.sql`): `chrlit_start` (999 Kč/**20** kr, image+carousel), `chrlit_rust` (2 999 Kč/**70** kr, +post_edit +post_variant +growth_tracking; **bez reels od 9/2026**), `chrlit_dominance` (4 999 Kč/**130** kr, +reel +product studio +product_line +priority 10), `chrlit_imperium` (8 999 Kč/**260** kr, +priority 20); `trial_v2` má `allowed_media: image+carousel`. Cena v DB je **měsíční**; delší období počítá `termPrice()` v `lib/pricing.ts`. Kredit ≈ $0.30 COGS: image 1 / carousel 3 / reel 5 (`lib/credits.ts`). Features JSON: `allowed_media[]` (chybí = vše povoleno, legacy), `growth_tracking` bool. Staré `chrlit` deaktivováno (grandfathered) |
...
| `REELS_ENABLED` | Optional (default OFF) | `=1` zapne reels — Seedance přes BytePlus ModelArk, vyžaduje `ARK_API_KEY` (kill-switch čtou `autopilot.ts`, `content-plan-actions.ts`, billing charge odhady). Zapnout až PO nasazení media-weighted kreditů |
