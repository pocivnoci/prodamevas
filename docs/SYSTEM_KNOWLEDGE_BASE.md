...
| `subscription_plans` | `id`, `name`, `price_czk`, `features` | Plan definitions — v4 media-weighted re-budget (`20260702_media_weighted_credits.sql`): `chrlit_start` (490 Kč/**20** kr, image+carousel), `chrlit_rust` (990 Kč/**45** kr, +post_variant +reel +growth_tracking), `chrlit_dominance` (1990 Kč/**110** kr, +product studio +priority); `trial_v2` má nově `allowed_media: image+carousel`. Kredit ≈ $0.30 COGS: image 1 / carousel 3 / reel od 5 (8 s; 16 s = 9, 24 s = 13 — `creditsForReel` dle délky, `lib/credits.ts`). Features JSON: `allowed_media[]` (chybí = vše povoleno, legacy), `growth_tracking` bool. Staré `chrlit` deaktivováno (grandfathered) |
...
| `video-processor.ts` | ~430 | ffmpeg reel assembly — processReelVideo() (multi-clip concat + audio mix + drawtext titulky), planClips(), groupScenesIntoClips(), scenesToSubtitles()/wrapSubtitleText()/escapeDrawtext(), pcmToWav()/wavDurationSec() (Gemini TTS returns headerless PCM), extractFrames() (frame QA). Font `instagram/assets/fonts/Inter-Bold.ttf` + ffmpeg-static binary traced via `outputFileTracingIncludes` |
| `orchestrators/media-refs.ts` | ~110 | loadProductPhoto() — shared product-photo loader (image + reel grounding) |
| `../lib/media-urls.ts` | ~60 | parsePostMedia(image_url, media_type) — the ONLY `image_url` pipe parser (reel `video|cover`, carousel slides); `media_type` wins over pipe-sniffing |
...
|-------|------|----------|---------|
| `POST /api/ig-create-job` | ✅ membership + rate limit | 10s | Create job, **charge media-weighted credit/plan counter** (image 1 / carousel 3 / reel od 5 dle délky; `chargedCredits`+`chargedMedium`+`chargedReelDuration` v job config; refunded on failure, down-clamp dorovnán přes `reconcileJobCharge`), return jobId |
...
| `REELS_ENABLED` | Optional (default OFF) | `=1` zapne Veo reels (kill-switch čtou `autopilot.ts` `applySafetyClamps`, `content-plan-actions.ts`, `campaign-worker`, `variant-actions`, billing charge; UI ho čte přes `/api/subscription` → `reelsEnabled` a schová „Reel" v pickeru dokud OFF). Reel pipeline (voiceover+titulky+produkt+multi-clip+publishing) je od v8.1 KOMPLETNÍ — flip na `=1` je jediná launch akce, žádný redeploy |
...
| Voiceover (Gemini TTS) | gemini-3.1-flash-tts | ~$0.02 |
| Frame QA per clip (3 frames) | gemini-pro-latest (visionQA) | ~$0.01 |
| **Total per image post** | — | **~$0.27** |
| **Total per reel 8s (fast)** | — | **~$1.45** (16s ~$2.85 · 24s ~$4.15 — multi-clip) |
