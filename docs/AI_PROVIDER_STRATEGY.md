...
Fact Gate ─ TRUTH GATE: claims vs. brandFacts (Claude judge)     fact-check.ts (checkCaptionFacts)
   └ unsure & about the WORLD → web search (Claude Haiku)         fact-web.ts (verifyClaimsOnWeb)
...
- **Fact gate** sees the finished copy plus the ALLOWED SOURCES only: `config.brandFacts`, the selected product (live catalog), the idea/topic, the review quote, and anything this post already had verified on the web. Everything concrete outside them (number, year, price, guarantee, superlative, third-party name) is `risk` → replaced deterministically by substring swap; what survives is logged as `fact_status='flagged'` and flagged on the post card. Everyday narrative colour (time of day, weather, sensory detail) and timeless textbook constants are explicitly NOT claims — the gate must not turn concrete posts into mush. Runs after the board and **before** the render, because the hook is about to be burnt into the image.
- **Plan hooks go through the same gate** (`content-plan-actions.ts`, via `checkDisplayStrings`) before the preview is shown, and again on a single-item regenerate. This closes a laundering path: an approved hook is an ALLOWED SOURCE for the post's gate, and that is only honest if the user could see what they were approving. A hook the user approves *despite* a warning keeps its wording (the mega prompt promised that) but carries the warning into `fact_flags` on the post — approval protects the words, not the label.
- **Web verifier** (`fact-web.ts`, Claude Haiku 4.5 + the `web_search` server tool) gets ONLY the claims the judge marked `unsure` **and** scoped to the world — a law, a norm, a deadline, a third party's product spec. It can do exactly one thing: move a claim from unsure to proven and attach a citation. It cannot rewrite text and it cannot overturn a claim the Pro judge passed, which is why a cheap tier is safe here. A URL is accepted only if it appears in the API's actual search results, and never if it is on the client's own domain — backing a claim about the client with the client's own marketing is the circular "verified" stamp this whole layer exists to prevent. Claims **about the brand itself** never go to the web at all; those belong in Ověřená fakta, entered by a person. Fails closed: no key, no result, any error → the claim stays unsupported, i.e. the pre-web behaviour. Results land in `ig_generation_log.fact_sources` and are shown at the post as real links.
