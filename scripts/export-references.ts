...
function emojiFor(industry: string, name: string): string {
    ...
    if (s.includes("řemesl") || s.includes("remesl") || s.includes("rekonstru") || s.includes("stav")) return "🔨"
    if (s.includes("realit") || s.includes("nemovit")) return "🏡"
    if (s.includes("zdrav") || s.includes("dentál") || s.includes("dental") || s.includes("zub") || s.includes("estetik")) return "🦷"
    if (s.includes("poraden") || s.includes("koučin") || s.includes("koucin") || s.includes("finanč") || s.includes("financ")) return "📈"
