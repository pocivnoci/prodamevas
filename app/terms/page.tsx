        ...
        <div className="min-h-screen bg-[#050505] text-white">
            <div className="max-w-3xl mx-auto px-6 py-24">
                ...
                <div className="prose prose-invert max-w-none space-y-10 text-white/70 text-sm leading-relaxed">
                    ...
                    <Section n={6} title="Ceny, zúčtovací období a platební podmínky">
                        <p>Ceny tarifů jsou uvedeny v ceníku na hlavní stránce a v aplikaci. <strong className="text-white/90">{vatNotice()}</strong></p>
                        <p className="mt-3">Uživatel si při objednávce volí <strong className="text-white/90">délku zúčtovacího období: 1, 3, 6 nebo 12 měsíců</strong>. U delších období je cena za měsíc nižší; celková cena za období je vždy uvedena u tarifu před dokončením objednávky. Kredity se přitom obnovují <strong className="text-white/90">každý měsíc</strong> bez ohledu na délku zaplaceného období.</p>
                        <p className="mt-3">Platba probíhá <strong className="text-white/90">předem za celé zvolené období</strong> prostřednictvím platební brány ComGate Payments, a.s. nebo Stripe Payments Europe, Ltd. Poskytovatel neuchovává údaje o platební kartě; ty zpracovává výhradně platební brána.</p>
                        <p className="mt-3">Na každou přijatou platbu vystaví poskytovatel daňový doklad (fakturu) s uvedením zaplaceného období, který zašle na e-mail uživatele a zpřístupní v aplikaci. Fakturační údaje zadává uživatel před první platbou a odpovídá za jejich správnost.</p>
                    </Section>

                    <Section n={7} title="Kredity a limity">
                        <p>Rozsah generování je řízen kredity. Každý tarif zahrnuje měsíční příděl kreditů, který se obnovuje na začátku každého kreditového období. Kreditové období je <strong className="text-white/90">vždy měsíční, i když je zaplaceno na 3, 6 nebo 12 měsíců dopředu</strong> — předplacením delšího období tedy uživatel nezískává kredity dopředu, ale nižší cenu za měsíc. Různé typy obsahu spotřebují různý počet kreditů (obrázek 1, story 2, carousel 3, reel 5); aktuální váhy jsou uvedeny v aplikaci.</p>
                        <p className="mt-3"><strong className="text-white/90">Nevyčerpané kredity se nepřevádějí do dalšího měsíce a jejich nevyužitím zanikají</strong> — a to i u předplatného zaplaceného na delší období. Kredity nelze směnit za peníze ani převést na jiný účet.</p>
                    ...
                    <Section n={8} title="Automatická obnova a ukončení předplatného">
                        <p>Předplatné se <strong className="text-white/90">automaticky obnovuje vždy na stejně dlouhé období</strong>, jaké si uživatel zvolil, za cenu platnou v okamžiku obnovy, dokud jej uživatel nezruší. O nadcházející obnově i o změně ceny informuje poskytovatel e-mailem předem — u měsíčního předplatného nejméně 3 dny předem, u předplatného na 3 měsíce a déle <strong className="text-white/90">nejméně 30 dní předem</strong>.</p>
                        <p className="mt-3">Uživatel může předplatné kdykoli zrušit v aplikaci v sekci Předplatné, a to bez udání důvodu a bez sankce. Zrušení je účinné ke konci již zaplaceného období — do té doby zůstává služba plně přístupná. Zaplacené období se nevrací poměrnou částí, není-li dále uvedeno jinak (viz článek 9).</p>
                    ...
                    <Section n={9} title="Garance vrácení peněz do 30 dnů">
                        <p>Nad rámec zákonných práv poskytuje poskytovatel <strong className="text-white/90">dobrovolnou garanci vrácení peněz do 30 dnů</strong> od první platby za zvolené zúčtovací období, a to <strong className="text-white/90">bez udání důvodu</strong>. Garance se vztahuje na všechna zúčtovací období, tedy i na předplatné zaplacené na 3, 6 nebo 12 měsíců dopředu.</p>
                        <p className="mt-3">O vrácení peněz stačí požádat e-mailem na <a href={`mailto:${LEGAL.email}`} className="text-white/80 underline">{LEGAL.email}</a>. Poskytovatel vrátí <strong className="text-white/90">celou zaplacenou částku</strong> do 14 dnů od žádosti stejným způsobem, jakým platbu přijal, a k témuž okamžiku ukončí předplatné. Vygenerovaný obsah zůstává uživateli.</p>
                        <p className="mt-3">Garance se vztahuje na <strong className="text-white/90">první platbu</strong> za dané zúčtovací období, nikoli na jeho automatické obnovy — o nadcházející obnově poskytovatel informuje předem podle článku 8 a lze ji do té doby bezplatně zrušit. Garanci lze uplatnit jednou za účet.</p>
                        <p className="mt-3">Tato garance nijak nezkracuje zákonné právo spotřebitele podle článku 10 ani práva z vadného plnění podle článku 11.</p>
                    </Section>

                    <Section n={10} title="Odstoupení od smlouvy (spotřebitelská)">
                    ...
                    <Section n={11} title="Vady služby a reklamace">
                        ...
                        {/* Slib z ceníku patří i sem, jinak je to jen marketing.
                            Hlídají aserce 13.16 a 13.17. */}
                        <p className="mt-3">Podpora je dostupná e-mailem na <a href={`mailto:${LEGAL.email}`} className="text-white/80 underline">{LEGAL.email}</a>{LEGAL.phone ? <> a telefonicky na <a href={`tel:${LEGAL.phone.replace(/\s/g, "")}`} className="text-white/80 underline">{LEGAL.phone}</a></> : null} v pracovní dny. V tarifu <strong className="text-white/90">Impérium</strong> se dotazy uživatele vyřizují přednostně před ostatními tarify a vygenerovaný obsah pravidelně prochází kontrolou marketingového specialisty poskytovatele. U ostatních tarifů není doba odpovědi ujednána. Tím není dotčena lhůta pro vyřízení reklamace podle předchozího odstavce.</p>
                    </Section>

                    <Section n={12} title="Práva k obsahu">
                    ...
                    <Section n={13} title="Zakázané užití">
                    ...
                    <Section n={14} title="Dostupnost a omezení odpovědnosti">
                        ...
                        <p className="mt-3">Poskytovatel neodpovídá za výpadky služeb třetích stran (zejména Google, Supabase, Vercel, Meta, ComGate, Stripe), které mohou dočasně ovlivnit funkčnost, ani za změny jejich podmínek či cen.</p>
                    ...
                    <Section n={15} title="Ochrana osobních údajů">
                    ...
                    <Section n={16} title="Mimosoudní řešení sporů (spotřebitelská)">
                    ...
                    <Section n={18} title="Závěrečná ustanovení">
