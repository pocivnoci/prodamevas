...
export function ConsultationSection({ projectId }: { projectId: string }) {
    const t = useTranslations("billing.consultation")
    ...
    const buy = async () => {
        ...
        try {
            ...
            else { checkout.abort(); setError(data.error || t("errors.payment")) }
        } catch {
            checkout.abort()
                ...
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs text-emerald-400 font-bold mt-2">
                            {t("booked.confirmed", {
                                date: Number.isNaN(when.getTime()) ? "" : format.dateTime(when, { dateStyle: "long", timeStyle: "short" }),
                            })}
                        </p>
    ...
    if (state.status === "paid" || state.status === "entitled") {
        return (
            <Card highlight>
                <div className="flex items-start justify-between gap-4">
                    <div>
                        <Title badge={state.included ? t("entitled.included") : t("entitled.paid")} />
                        <p className="text-[10px] text-white/40 mt-2 leading-relaxed max-w-md">
                            {state.bookingReady
                                ? t("entitled.pickSlot", { minutes: state.durationMinutes })
                                : t("entitled.weWillReachOut")}
                        </p>
                    </div>
                    {state.bookingReady && (
                        <button
                        ...
                        >
                            {busy ? t("entitled.opening") : t("entitled.pick")}
                        </button>
                    )}
                </div>
        ...
        <Card>
            <div className="flex items-start justify-between gap-4">
                <div>
                    <Title />
                    <p className="text-[10px] text-white/40 mt-2 leading-relaxed max-w-md">
                        {t("offer.pitch", { minutes: state.durationMinutes })}
                    </p>
                ...
                <div className="shrink-0 text-right">
                    ...
                    {LEGAL.vatStatus === "payer" && (
                        <p className="text-[8px] text-white/25 font-bold uppercase tracking-widest">
                            {t("offer.vat", { gross: formatCzk(chargeableHaleru(state.priceHaleru)) })}
                        </p>
                    )}
                    <button
                        onClick={buy}
                    ...
                    >
                        {busy ? t("offer.processing") : t("offer.order")}
                    </button>
                </div>
            </div>
            {error && <p className="text-[10px] text-red-400 mt-3">{error}</p>}
