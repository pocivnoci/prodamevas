...
export function ReviewsTab({ projectId }: { projectId: string }) {
    ...
    const t = useTranslations("inspiration.reviews")
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold tracking-widest uppercase text-white/40">{t("count", { count: reviews.length })}</span>
            ...
            {showForm && (
                <div className="bg-[#0f0f0f] border border-white/10 rounded-sm p-6 space-y-4 shadow-lg shrink-0">
                    ...
                    <div className="flex gap-3">
                        <input
                            type="text"
                            placeholder={t("form.initialsPlaceholder")}
                    ...
                    <div key={review.id} className="bg-[#0a0a0a] border border-white/5 rounded-sm p-4 hover:border-white/10 transition-all hover:bg-white/5 shadow-sm">
                        <div className="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <p className="text-[10px] text-white/40 mt-1.5 font-bold uppercase tracking-widest">— {review.customer_initials || t("anonymous")} • {"⭐".repeat(review.rating || 5)}</p>
                            </div>
                            <div className="flex items-center gap-2 flex-shrink-0">
                                {review.is_approved ? (
                                    <span className="inline-flex items-center gap-1.5 text-[9px] px-2 py-1 rounded-sm bg-emerald-500/10 text-emerald-400 uppercase tracking-widest font-bold border border-emerald-500/20"><Check className="w-3 h-3 shrink-0" />{t("approved")}</span>
                                ) : (
            ...
            {reviews.length === 0 && (
                <div className="text-center py-12 text-white/40">
                    ...
                    <p className="text-[10px] uppercase font-bold tracking-widest">{t("empty")}</p>
                </div>
            )}
        </div>
    )
