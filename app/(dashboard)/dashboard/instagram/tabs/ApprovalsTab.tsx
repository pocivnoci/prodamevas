...
export function ApprovalsTab({ projectId }: { projectId: string }) {
    ...
    const load = useCallback(async () => {
        ...
        try { setItems(await getPendingApprovals(projectId)) } catch { setItems([]) }
        setLoading(false)
    }, [projectId])
    ...
    const decide = async (id: string, approve: boolean) => {
        setBusy(id)
        const res = approve ? await approveAgentAction(projectId, id) : await rejectAgentAction(projectId, id)
