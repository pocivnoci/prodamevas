...
export function TasksTab() {
    ...
    useEffect(() => {
        ...
        Promise.all([listTasks(), listTeam(), listClientsForTasks(), currentUserEmail()])
            ...
            .catch(err => { if (alive) { setError((err as Error)?.message || "Načtení selhalo."); setLoading(false) } })
        return () => { alive = false }
    }, [])
    ...
    const runAgent = async () => {
        ...
        if (!res.success) {
            setAgentNote(res.error ?? "Spuštění selhalo.")
            setAgentRunning(false)
            return
        }
        setAgentNote("AI se do toho pustila — seznam se doplní sám během chvilky.")
        pollTimers.current.forEach(clearTimeout)
        pollTimers.current = [10_000, 20_000, 30_000].map((delay, i) =>
            setTimeout(() => {
                void reload()
                if (i === 2) { setAgentRunning(false); setAgentNote("Hotovo — seznam je aktuální.") }
            }, delay))
    }

    const runRetriage = async (task: Task) => {
        const ok = await run(task, () => retriageTask(task.id), "Přetřídění selhalo.")
    ...
    const groups: { key: string; label: string; items: Task[]; tone?: string; collapsed?: boolean }[] = [
        { key: "awaiting", label: "Čeká na tvou odpověď", items: sections.awaiting, tone: "text-amber-300" },
        { key: "mine", label: "Moje", items: sections.mine },
        { key: "unassigned", label: "Nezařazené", items: sections.unassigned },
        { key: "others", label: "Ostatní otevřené", items: sections.others },
    ]
        ...
        <div className="space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-2">
                <span className="text-[11px] font-bold uppercase tracking-widest text-white/60">
                    Otevřených <span className="text-white/25">{openCount}</span>
                </span>
                <input
                    value={q}
                ...
                ><Plus className="w-3.5 h-3.5" />Nový úkol</button>
            </div>

            <div className="flex flex-wrap items-center gap-1.5">
                <FilterChip active={ownerFilter === "all"} onClick={() => setOwnerFilter("all")} label={`Vše (${openCount})`} />
                {team.filter(m => m.active).map(m => (
                    <FilterChip
                        key={m.email}
                        active={ownerFilter === m.email}
                        onClick={() => setOwnerFilter(m.email)}
                        label={m.email === me ? `${m.name} (já)` : m.name}
                    />
                ))}
                <FilterChip active={ownerFilter === "none"} onClick={() => setOwnerFilter("none")} label="Nezadáno" />
                <button
                    onClick={runAgent}
            ...
            {visible.length === 0 && (
                <p className="text-white/30 text-xs py-8 text-center">
                    Nic tu není. Přidej úkol, nebo zruš filtr.
                </p>
            )}
            ...
            {sections.closed.length > 0 && (
                <div className="space-y-1.5">
                    ...
                    >
                        Hotové a zahozené <span className="text-white/20">({sections.closed.length})</span> {showDone ? "▲" : "▼"}
                    </button>
    ...
    return (
        <div className="space-y-1.5">
            <button
                ...
                className="text-[10px] font-bold uppercase tracking-widest text-white/40 hover:text-white transition-colors"
            >
                Blokované <span className="text-white/20">({tasks.length})</span> {open ? "▲" : "▼"}
            </button>
            {open && (
                <>
                    {awake.map(render)}
                    {sleeping.length > 0 && (
                        ...
                        >
                            {showSleeping ? "Skrýt" : "Zobrazit"} {sleeping.length}× s termínem v budoucnu
                        </button>
                    )}
        ...
        >
            <div className="flex items-start gap-3 px-3 py-2.5">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {!muted && task.blocked_on && (
                        <p className="text-[10px] text-amber-300/80 mt-1">
                            ⏸ {task.blocked_on}
                            {task.blocked_until && <span className="text-white/30"> · do {czDate(task.blocked_until)}</span>}
                        </p>
                    )}
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                        ...
                        {task.due_date && (
                            <span className={`text-[8px] font-bold uppercase tracking-widest ${overdue ? "text-red-400" : "text-white/35"}`}>
                                {overdue ? "po termínu " : "termín "}{czDate(task.due_date)}
                            </span>
                        )}
                        <span className="text-[8px] font-bold uppercase tracking-widest text-white/20">
                            {task.created_by === "ai" ? "návrh AI" : task.source === "sheet" ? "z tabulky" : "v appce"}
                        </span>
                        {task.effort && (
                ...
                <select
                    ...
                    onChange={e => void onRun(() => assignTask(task.id, e.target.value || null), "Přiřazení selhalo.")}
                    disabled={busy}
    ...
    return (
        <div className="border-t border-white/5 px-3 py-3 space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-x-4 gap-y-3">
                <Field label="Název" value={task.title} onSave={v => void onRun(() => updateTask(task.id, { title: v }), "Uložení selhalo.")} />
                <Select
                    label="Priorita"
                    value={task.priority ? String(task.priority) : null}
                    options={PRIORITY_OPTIONS}
                    emptyLabel="bez priority"
                    onSave={v => void onRun(() => updateTask(task.id, { priority: v ? Number(v) : null }), "Uložení selhalo.")}
                />
                <DateField
                    label="Termín"
                    mode="date"
                    value={task.due_date}
                    onSave={v => void onRun(() => updateTask(task.id, { dueDate: v || null }), "Uložení selhalo.")}
                />
                <Select
                    label="Klient"
                    value={task.client_id}
                    options={clientOptions}
                    emptyLabel="firemní úkol"
                    onSave={v => void onRun(() => setTaskClient(task.id, v || null), "Přiřazení klienta selhalo.")}
                />
                <div className="sm:col-span-2">
                    <Field label="Poznámka" value={task.note} multiline onSave={v => void onRun(() => updateTask(task.id, { note: v }), "Uložení selhalo.")} />
                </div>
            </div>
            ...
            <div className="grid sm:grid-cols-[1fr_auto_auto] gap-2 items-end">
                <label className="block">
                    <span className="block text-[8px] font-bold uppercase tracking-widest text-white/30 mb-1">Čekám na</span>
                    <input
                        value={blockDraft}
                        onChange={e => setBlockDraft(e.target.value)}
                        placeholder="Např. verifikace v bance"
                ...
                <DateField
                    ...
                    onSave={v => void onRun(() => setTaskBlocked(task.id, blockDraft || task.blocked_on, v || null), "Odložení selhalo.")}
                />
                <div className="flex gap-2">
                    <button
                        onClick={() => void onRun(() => setTaskBlocked(task.id, blockDraft, task.blocked_until), "Odložení selhalo.")}
                    ...
                    {task.blocked_on && (
                        <button
                            onClick={() => { setBlockDraft(""); void onRun(() => setTaskBlocked(task.id, null, null), "Odblokování selhalo.") }}
                            disabled={busy}
            ...
            <div className="flex gap-2">
                ...
                />
                <button
                    onClick={() => void onRun(() => setTaskResult(task.id, resultDraft), "Uložení výsledku selhalo.")}
            ...
            <div className="flex flex-col sm:flex-row gap-2">
                <input
                    ...
                    placeholder={isQuestionForHuman(task.blocked_on) ? "Odpověz AI jednou větou…" : "Poznámka do vlákna…"}
                    className="flex-1 bg-[#050505] border border-white/10 rounded-sm px-2.5 py-1.5 text-white/80 text-[11px] focus:outline-none focus:ring-1 focus:ring-white/20 placeholder:text-white/20"
                />
                <button
            ...
            <div className="flex items-center gap-3 pt-1">
                <span className="text-[9px] text-white/20 uppercase tracking-widest font-bold">
                    {task.source === "sheet" ? "Z tabulky · ruční import ho může vrátit" : "Založeno v appce"}
                </span>
                <button
...
function NewTaskForm({ team, clients, defaultOwner, onDone }: {
    ...
    return (
        <div className="border border-white/10 bg-[#050505] rounded-sm p-3 space-y-2">
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Co je potřeba udělat *" autoFocus className={`w-full ${inputClass}`} />
            ...
            <div className="grid sm:grid-cols-4 gap-2">
                ...
                <select value={priority} onChange={e => setPriority(e.target.value)} className={inputClass}>
                    ...
                    {Object.entries(PRIORITY_OPTIONS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                </select>
                <input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className={inputClass} />
                <select value={clientId} onChange={e => setClientId(e.target.value)} className={inputClass}>
                    <option value="">Firemní úkol</option>
                    {clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
            </div>
            <div className="flex items-center gap-2">
                ...
                <span className="text-[8px] uppercase tracking-widest font-bold text-white/20 ml-auto text-right">
                    AI ho roztřídí sama při nejbližším běhu
                </span>
            </div>
            {error && <p className="text-[10px] text-red-400">{error}</p>}
        </div>
