...
/**
 * Nadpis i popisek sekce jdou z messages (`nav.items.<id>.label`,
 * `sections.descriptions.<id>`), klíč nadpisu nese registr navigace
 * (`navItem(id).label`). Dvě kopie názvu se dřív rozešly: `leads` ani `emails`
 * v tabulce popisků nebyly a obě sekce se otevíraly bez nadpisu. Sekce bez
 * vlastní položky v registru (sub-tab) má nadpis v `sections.subsections.<id>`.
 */
...
export default function InstagramPage() {
    ...
    const labelKey = navItem(activeSection)?.label
    const sectionInfo = {
        title: labelKey
            ? tNav(labelKey)
            : tSections.has(`subsections.${activeSection}`) ? tSections(`subsections.${activeSection}`) : "",
        description: tSections.has(`descriptions.${activeSection}`) ? tSections(`descriptions.${activeSection}`) : "",
    }
