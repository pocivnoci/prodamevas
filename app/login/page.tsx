...
export default async function LoginPage(props: {
    ...
    // Chybové klíče posílají akce i `middleware.ts` (session_error na chráněné
    // cestě). Neznámý klíč = obecná hláška, nikdy holý klíč v UI.
    const errorMessage = errorKey
        ? (t.has(`login.errors.${errorKey}`) ? t(`login.errors.${errorKey}`) : t("login.fallbackError"))
        : null
        ...
        <div className="min-h-screen flex items-center justify-center bg-[#050505] p-4 text-white">
            <div className="w-full max-w-md p-8 bg-[#0a0a0a] border border-white/10 rounded-sm">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-xl font-black uppercase tracking-widest">{t("login.title")}</h1>
                    <p className="text-white/40 mt-2 text-xs font-medium">{t("login.subtitle")}</p>
                </div>

                {errorMessage && (
                    <AuthNotice tone="error" title={t("common.failed")}>{errorMessage}</AuthNotice>
                )}

                {googleAuthEnabled() && (
                    <>
                        <form>
                            <GoogleButton action={signInWithGoogle} label={t("common.google")} />
                        </form>
